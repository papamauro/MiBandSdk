...
public class MiBand1MonitorDevice extends MonitorDevice{
    ...
    public void initialize() {
        ...
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_LE_PARAMS , MiBandService.getLowLatency());
        LOG.info(2 + ") reading time");
        btClient.readCharacteristic(MiBandService.UUID_CHARACTERISTIC_DATE_TIME);
        LOG.info(3 + ") pairing");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_PAIR , new byte[] {2});
        LOG.info(4 + ") reading device info");
        btClient.readCharacteristic(MiBandService.UUID_CHARACTERISTIC_DEVICE_INFO);
        LOG.info(5 + ") reading gap info");
        btClient.readCharacteristic(MiBandService.UUID_CHARACTERISTIC_GAP_DEVICE_NAME);
        LOG.info(6 + ") write user info");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_USER_INFO, new byte[] {-10 , -28 , 99 , 92 , 0 , 0 , -81 , 70 , 0 , 4 , 0 , 49 , 53 , 53 , 48 , 48 , 53 , 48 , 53 , 37 } );
        LOG.info(7 + ") write location");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_CONTROL_POINT , new byte[]{
        ...
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_HEART_RATE_CONTROL_POINT, MiBandService.startHeartMeasurementSleep);


        LOG.info(9 + ") registering ");
        btClient.registerToCharacteristic(MiBandService.UUID_CHARACTERISTIC_HEART_RATE_MEASUREMENT, true );
        btClient.registerToCharacteristic(MiBandService.UUID_CHARACTERISTIC_REALTIME_STEPS, true );
        btClient.registerToCharacteristic(MiBandService.UUID_CHARACTERISTIC_ACTIVITY_DATA, true );
        btClient.registerToCharacteristic(MiBandService.UUID_CHARACTERISTIC_BATTERY, true );
        btClient.registerToCharacteristic(MiBandService.UUID_CHARACTERISTIC_SENSOR_DATA, true );

        LOG.info(14 + ") setting time ");
        setCurrentTime();

        LOG.info(15 + ") read battery ");
        btClient.readCharacteristic(MiBandService.UUID_CHARACTERISTIC_BATTERY);

        LOG.info(16 + ") set high latency ");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_LE_PARAMS , MiBandService.getHighLatency());

        //added just to try something
        LOG.info(17 + ") vibrate");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_CONTROL_POINT, MiBandService.getDefaultNotification());

        LOG.info(18 + ") vibrate");
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_CONTROL_POINT, MiBandService.startRealTimeStepsNotifications);
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_CONTROL_POINT, MiBandService.startSensorRead);
    ...
    BTSequentialClient.CharacteristicChangedListener characteristicChangedListener = new BTSequentialClient.CharacteristicChangedListener() {
        ...
        public void onCharacteristicChanged(BluetoothGattCharacteristic characteristic) {
            if(UIDS.UUID_CHARACTERISTIC_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
                LOG.info("received heartbeat characteristic: " + Arrays.toString(characteristic.getValue()));
                int heartbeat = (int)characteristic.getValue()[1];
                if(heartbeat > 0 && heartrateListener!=null) {
                    heartrateListener.update(heartbeat);
                }
            }
        }
    };

    ValueListener heartrateListener;

    public void startHeartMonitoring(ValueListener listener) {
        LOG.info("starting continuous heart monitoring");
        heartrateListener = listener;
        btClient.addCharacteristicChangedListener(characteristicChangedListener);
        addPeriodicWrite(MiBandService.UUID_CHARACTERISTIC_HEART_RATE_CONTROL_POINT.toString(), MiBandService.startHeartMeasurementContinuous);
    }

    public void stopHeartMonitoring(){
        LOG.info("stopping continuous heart monitoring");
        removePeriodicWrite(MiBandService.UUID_CHARACTERISTIC_HEART_RATE_CONTROL_POINT.toString());
        btClient.removeCharacteristicChangedListener(characteristicChangedListener);
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_HEART_RATE_CONTROL_POINT, MiBandService.stopHeartMeasurementContinuous);
    }

    public void vibrate(){
        btClient.writeCharacteristic(MiBandService.UUID_CHARACTERISTIC_CONTROL_POINT, UIDS.getDefaultNotification());
        btClient.writeCharacteristicNoWait(GattCharacteristic.UUID_CHARACTERISTIC_ALERT_LEVEL, new byte[]{GattCharacteristic.MILD_ALERT});
    }
	...
	*/
    ...
    public static byte[] getDefaultNotification() {
        ...
        public static byte[] getNotification() {
            return getNotification(250l, 1, 1, 0xFFFFFFFF, 0xFFFFFFFF, 250l);
        }
